...
export function HologramFactory<T extends keyof schema.HologramClasses>({ uri, type, settings }: { uri: string; type: T; settings: schema.HologramSettings[T]}) {
    const ArgsSchema = schema.hologramMap[type];
    ArgsSchema.parse(settings);
    switch(type) {
        case "quilt":
